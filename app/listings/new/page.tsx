...
export default function NewListingPage() {
  ...
  const { requireAuth, loadSavedFormData, isCheckingAuth } = useAuthRedirect();

  // Load saved form data after component mounts and when auth status changes
  useEffect(() => {
    if (formRef.current && status === 'authenticated') {
      loadSavedFormData(formRef.current);
    }
  }, [status, loadSavedFormData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Only submit if we're on the media tab
    if (currentTab !== 'media') {
      return;
    }
    ...
    console.log('Submitting form, auth status:', status);

    // Check authentication status on client side first
    if (status === 'loading') {
      return;
    }
    ...
    try {
      // Create FormData from state, not from the DOM form element
      // because tabs unmount inputs, so they wouldn't be included in new FormData(form)
      ...
      submitData.append('verified', String(formData.verified));
      ...
      const redirectId = result.listing.slug || result.listing.id || result.listing._id;
      if (redirectId) {
        router.push(`/listings/${redirectId}`);
      } else {
        console.error('No ID or slug returned for new listing');
        toast.error('Listing created but could not redirect');
      }
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Share your business with our community. Fill in the details to get started and reach more customers.
          </p>
        </div>
        ...
        <div className="bg-yellow-100 p-4 mb-4 rounded text-xs font-mono">
          <p>Status: {status}</p>
          <p>User: {session?.user ? JSON.stringify(session.user) : 'None'}</p>
        </div>
        ...
        <div className="bg-white shadow-xl rounded-2xl overflow-hidden">
          ...
          <form
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                // Prevent Enter key from submitting the form, except on the submit button itself
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          // Remove https:// if user accidentally types it
